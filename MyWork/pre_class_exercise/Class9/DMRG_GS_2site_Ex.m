...
for itS = (1:Nsweep)
    ...
    for itN = (N-1:-1:1)
        ...
        [U, Sv{itN}, M{itN+1}] = svdTr(Anew, 4, [1, 3], Nkeep, Skeep);
        % update the next tensor
        M{itN} = contract(U, 3, 3, diag(Sv{itN}), 2, 1, [1 3 2]);
        % update the Hamiltonian in effective basis
        Hlr{itN+2} = updateLeft(Hlr{itN+3},3, ...
            permute(M{itN+1},[2 1 3]), ...
            permute(Hs{itN+1},[1 2 4 3]), 4, ...
            permute(M{itN+1},[2 1 3]));
        Hlr{itN+1} = updateLeft(Hlr{itN+2}, 3, ...
            permute(M{itN},[2 1 3]), ...
            permute(Hs{itN},[1 2 4 3]), 4, ...
            permute(M{itN},[2 1 3]));
    ...
    for itN = (1:N-1)
        ...
        % SVD; now we can safely truncate small singular values;
        [M{itN}, Sv{itN+1}, Vd] = svdTr(Anew, 4, [1, 3], Nkeep, Skeep);
        M{itN} = permute(M{itN},[1 3 2]);

        % update the next tensor
        M{itN+1} = contract(diag(Sv{itN+1}), 2, 2, Vd, 3, 1);

        % update the Hamiltonian in effective basis
        Hlr{itN+1} = updateLeft(Hlr{itN}, 3, ...
            M{itN}, ...
            Hs{itN}, 4, ...
            M{itN});
        Hlr{itN+2} = updateLeft(Hlr{itN+1}, 3, ...
            M{itN+1}, ...
            Hs{itN+1}, 4, ...
            M{itN+1});
...
for itn = (1:nKrylov)
    ...
    Amul = contract(Amul, 5, [5, 3], Hcen2,4, [3,2] );
    Amul = contract(Amul, 5, [2, 5], Hright, 3, [2, 3], [1 4 2 3]); % w_itn

    alphas(itn) = real(contract(conj(As(:,:,:,:, itn)),4,(1:4),Amul,4,(1:4))); % alpha_itn = v_itn'*w_itn
    cnt = cnt+1; 
    if itn<nKrylov
        for it2 = (1:2) % do twice to further reduce numerical noise
            T = contract(conj(As(:,:,:,:, 1:itn)),5,(1:4), Amul, 4, (1:4));
            Amul = Amul - contract(As(:,:,:,:,1:itn),5,5,T,2,1); % u = w- v[n]*(v[n]'*w_itn) 
        end 

        %calculate norm 
        Anorm = sqrt(abs(contract(conj(Amul),4,(1:4),Amul,4,(1:4))));
