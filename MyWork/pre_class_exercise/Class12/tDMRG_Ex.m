...
% unitary on even bonds. Otherwise, on odd bonds.
    ...
    isright = mod(it1, 2);
    [M,EE(it1, :),dw(it1, :)] = tDMRG_1sweep(M,expHtmp,Nkeep, isright);
    
    if mod(it1,3) == 0
        % evaluate local expectation values
        Ovals(ceil(it1/3), :)=  tDMRG_expVal(M,O, isright);
...
if isright % left -> right
    for it = (1:N-1)
        state = contract(M{it}, 3, 2, M{it+1}, 3, 1, [1 3 2 4]);
        if ~isempty(expH{it})
            state = contract(state, 4, [3, 4], expH{it},4, [3, 4]);
        end
        [u, s, vd, dw(it)] = svdTr(state, 4, [1 3], Nkeep, Skeep);
        EE(it) = - sum(s.^2 .* log2(s.^2));
